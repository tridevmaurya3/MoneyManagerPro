...
public final class VisibleDataToolsController {
    ...
    private void inject() {
        ...
        toolbar.setTag(TAG);
        wrapper.addView(toolbar, new LinearLayout.LayoutParams(-1, dp(50)));
    ...
    private LinearLayout buildToolbar() {
        ...
        for (Range range : Range.values()) actions.addView(button(range.label, v -> selectRange(range)));
        actions.addView(button("Sort", v -> cycleSort()));
        actions.addView(button("PDF", v -> export(false, false)));
        actions.addView(button("Excel", v -> export(true, false)));
        actions.addView(button("Share PDF", v -> export(false, true)));
        scroll.addView(actions);
        outer.addView(scroll, new LinearLayout.LayoutParams(0, -1, 1f));
        return outer;
    }

    private MaterialButton button(String label, View.OnClickListener listener) {
    ...
    private enum Range { TODAY("Today"), THIS_WEEK("This Week"), THIS_MONTH("This Month"), LAST_MONTH("Last Month"), LAST_TWO_MONTHS("Last Two Month"), LAST_THREE_MONTHS("Last Three Month"), LAST_SIX_MONTHS("Last Six Month"), CUSTOM("Custom"); final String label; Range(String l) { label = l; } }
